...
public abstract class DecalParticle extends Particle {
    ...
    public void render(VertexConsumer vertex, Camera camera, float pticks) {
        ...
        vertex.addVertex(mat, size,0,size).setColor(1f,1f,1f,alpha).setUv(0,0).setLight(light).setOverlay(OverlayTexture.NO_OVERLAY);
        vertex.addVertex(mat, size,0,-size).setColor(1f,1f,1f,alpha).setUv(0,1).setLight(light).setOverlay(OverlayTexture.NO_OVERLAY);
        vertex.addVertex(mat, -size,0,-size).setColor(1f,1f,1f,alpha).setUv(1,1).setLight(light).setOverlay(OverlayTexture.NO_OVERLAY);
        vertex.addVertex(mat, -size,0,size).setColor(1f,1f,1f,alpha).setUv(1,0).setLight(light).setOverlay(OverlayTexture.NO_OVERLAY);
