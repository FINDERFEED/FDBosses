...
public class BossClientModEvents {
    ...
    public static void registerParticleFactories(RegisterParticleProvidersEvent event){
        ...
        event.registerSpriteSet(BossParticles.BIG_GEAR.get(), AnimatedSpriteParticle.Factory::new);
    }
    ...
    public static void addRenderers(EntityRenderersEvent.RegisterRenderers event){
        ...
        event.registerEntityRenderer(BossEntities.CLOCK_ATTACK.get(), FDEntityRendererBuilder.<ClockAttack>builder()
                        .addLayer(FDEntityRenderLayerOptions.<ClockAttack>builder()
                                ...
                                .build())
                        .freeRender(((clockAttack, partialTicks, v1, matrices, multiBufferSource, i) -> {

                            var attackTimings = clockAttack.getEntityData().get(ClockAttack.ATTACK_TIMINGS);
                            var percent = attackTimings.getAttackTimingPercent(0, clockAttack.afterRotatedTicks + v1);
                            FDColor color;
                            if (percent != 1){
                                ...
                                color = new FDColor(1,0.1f,0.1f,p * 0.5f);

                            }else {
                                color = new FDColor(0,0,0,0);
                            }

                            var tex = FDBosses.location("textures/entities/netzach/clock_attack_arrow.png");

                            float angle = FDMathUtil.lerp(clockAttack.previousRotationAngle, clockAttack.rotationAngle, v1);
